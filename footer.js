...
(function() {
    function initFooter() {
        if (document.getElementById('custom-bios-footer')) return; // Zabrání zdvojení

        var customFooter = document.createElement('footer');
        customFooter.id = 'custom-bios-footer';
        customFooter.innerHTML = 
          '<style>' +
          '#custom-bios-footer { background-color: #cccccc; color: #1a1a1a; padding: 60px 20px; font-family: "Source Sans 3", sans-serif; font-size: 15px; width: 100%; box-sizing: border-box; }' +
          ...
          '#custom-bios-footer .f-contact-block { margin-bottom: 20px; line-height: 1.5; }' +
        ...
        // Najde starou patičku, vloží novou PŘED ni a starou úplně skryje
        var existingFooter = document.getElementById('footer') || document.querySelector('footer');
        if (existingFooter) {
            existingFooter.parentNode.insertBefore(customFooter, existingFooter);
    ...
    if (document.readyState === 'loading') {
        ...
        initFooter(); // Pokud se kód načte později, rovnou jej spustí
    }
})();
